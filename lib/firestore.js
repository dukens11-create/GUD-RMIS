...
export async function getDriverDocuments(driverId) {
  const q = query(
    collection(db(), `${COLLECTIONS.DRIVERS}/${driverId}/documents`),
...
export async function deleteDriverDocument(driverId, docId) {
  await deleteDoc(
    doc(db(), `${COLLECTIONS.DRIVERS}/${driverId}/documents`, docId)
  );
}
...
export async function getVehicleDocuments(vehicleId) {
  const q = query(
    collection(db(), `${COLLECTIONS.VEHICLES}/${vehicleId}/documents`),
    orderBy('uploadedAt', 'desc')
  );
  const snap = await getDocs(q);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
...
export async function deleteVehicleDocument(vehicleId, docId) {
  await deleteDoc(
    doc(db(), `${COLLECTIONS.VEHICLES}/${vehicleId}/documents`, docId)
  );
}
