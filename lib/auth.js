...
import { auth, isFirebaseConfigured, missingFirebaseVars } from './firebase';
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
        <div className="w-full max-w-md rounded-2xl border border-red-200 bg-red-50 p-8 text-center shadow">
          ...
          <p className="mb-4 text-sm text-red-700">{firebaseError}</p>
          {missingFirebaseVars.length > 0 && (
            <div className="mt-2 rounded-lg border border-red-300 bg-white p-4 text-left">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-red-600">
                Missing environment variables:
              </p>
              <ul className="space-y-1">
                {missingFirebaseVars.map((v) => (
                  <li key={v} className="font-mono text-xs text-red-800">{v}</li>
                ))}
              </ul>
              <p className="mt-3 text-xs text-red-600">
                Set these in your hosting platform&apos;s Environment settings and redeploy.
              </p>
            </div>
          )}
        </div>
