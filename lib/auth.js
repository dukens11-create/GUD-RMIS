...
import { auth, isFirebaseConfigured, getMissingFirebaseEnvVars } from './firebase';
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    if (!isFirebaseConfigured()) {
      ...
      console.error(
        ...
        'embedded into the static build. Missing: ' + missing.join(', ')
      );
      setFirebaseError(missing);
      setLoading(false);
      return;
    }
  ...
  if (firebaseError) {
    const isMissingVars = Array.isArray(firebaseError);
    return (
          ...
          {isMissingVars ? (
            <>
              <p className="mb-3 text-sm text-red-700">
                Firebase is not configured. The following environment variables are missing:
              </p>
              <ul className="mb-4 space-y-1 text-left">
                {firebaseError.map((v) => (
                  <li key={v} className="rounded bg-red-100 px-3 py-1 font-mono text-xs text-red-800">
                    {v}
                  </li>
                ))}
              </ul>
              <p className="text-xs text-red-600">
                Set these variables on your hosting platform (e.g. Render → Environment) and redeploy.
              </p>
            </>
          ) : (
